"""
Flight Portal v4.1 - WATCHLIST SUPPORT (Multi-Size Display Support)
MatrixPortal flight display with tail number tracking and alerts
Supports 64x32, 64x64, and 128x64 RGB matrices
"""
...
# ⚠️  CONFIGURE YOUR MATRIX SIZE HERE
MATRIX_WIDTH = 64   # Options: 64, 128
MATRIX_HEIGHT = 64  # Options: 32, 64

# Validate configuration
if MATRIX_WIDTH not in [64, 128]:
    raise ValueError("MATRIX_WIDTH must be 64 or 128")
if MATRIX_HEIGHT not in [32, 64]:
    raise ValueError("MATRIX_HEIGHT must be 32 or 64")

print(f"Configured for {MATRIX_WIDTH}x{MATRIX_HEIGHT} display")
...
# Initialize MatrixPortal with size configuration
matrixportal = MatrixPortal(
    status_neopixel=board.NEOPIXEL,
    debug=True,
    width=MATRIX_WIDTH,
    height=MATRIX_HEIGHT,
    bit_depth=4,  # Lower bit depth for larger displays to save memory
)
...
def create_alert_border(display, flash_state):
    """Create flashing border for watchlist alerts (adaptive to display size)"""
    color = COLOR_RED if flash_state else COLOR_YELLOW

    # Border thickness scales with display size
    border_thickness = 2 if MATRIX_WIDTH <= 64 else 3
    ...
    # Bottom border
    for y in range(MATRIX_HEIGHT - border_thickness, MATRIX_HEIGHT):
        for x in range(MATRIX_WIDTH):
            pixel = displayio.TileGrid(
                bitmap=displayio.Bitmap(1, 1, 1),
                pixel_shader=displayio.Palette(1),
    ...
    # Left border
    for y in range(border_thickness, MATRIX_HEIGHT - border_thickness):
    ...
    # Right border
    for y in range(border_thickness, MATRIX_HEIGHT - border_thickness):
        for x in range(MATRIX_WIDTH - border_thickness, MATRIX_WIDTH):
            pixel = displayio.TileGrid(
                bitmap=displayio.Bitmap(1, 1, 1),
                pixel_shader=displayio.Palette(1),
                x=x, y=y
...
def display_watchlist_alert(flight, lat, lon):
    """Display special alert layout for watchlist aircraft (adaptive layout)"""
    ...
    # Calculate adaptive positioning based on display size
    border_padding = 2 if MATRIX_WIDTH <= 64 else 3
    text_scale = 1 if MATRIX_WIDTH <= 64 else 2

    # Vertical positioning (adaptive)
    if MATRIX_HEIGHT == 32:
        y_positions = [4, 11, 18, 24, 29]
    elif MATRIX_HEIGHT == 64:
        y_positions = [8, 20, 32, 44, 56]
    else:
        y_positions = [4, 11, 18, 24, 29]  # fallback
    ...
    matrixportal.add_text(
        text_position=(border_padding + 2, y_positions[0]),
        text_font=FONT,
    ...
    max_tail_length = 10 if MATRIX_WIDTH <= 64 else 15
    matrixportal.add_text(
        text_position=(border_padding + 2, y_positions[1]),
        text_font=FONT,
        text_color=COLOR_CYAN,
        text_scale=text_scale,
    )
    matrixportal.set_text(tail[:max_tail_length], 1)

    # Row 3: Aircraft type
    max_type_length = 10 if MATRIX_WIDTH <= 64 else 20
    matrixportal.add_text(
        text_position=(border_padding + 2, y_positions[2]),
        text_font=FONT,
        text_color=COLOR_WHITE,
        text_scale=text_scale,
    )
    matrixportal.set_text(aircraft_type[:max_type_length], 2)

    # Row 4: Flight data
    max_data_length = 15 if MATRIX_WIDTH <= 64 else 25
    flight_data = f"{int(altitude)}ft {int(speed)}kt {heading}"
    matrixportal.add_text(
        text_position=(border_padding + 2, y_positions[3]),
        text_font=FONT,
        text_color=COLOR_GREEN,
        text_scale=text_scale,
    )
    matrixportal.set_text(flight_data[:max_data_length], 3)

    # Row 5: Distance & direction
    distance_text = f"{distance}mi {direction}"
    matrixportal.add_text(
        text_position=(border_padding + 2, y_positions[4]),
        text_font=FONT,
        text_color=COLOR_WHITE,
        text_scale=text_scale,
    )
...
def display_normal_layout(flights):
    """Display normal flight tracking layout (adaptive)"""
    matrixportal.display.show(None)

    if not flights:
        y_center = MATRIX_HEIGHT // 2
        matrixportal.add_text(
            text_position=(2, y_center),
            text_font=FONT,
            text_color=COLOR_WHITE,
            text_scale=1 if MATRIX_WIDTH <= 64 else 2,
        )
    ...
    # Calculate how many flights fit based on display size
    max_flights = (MATRIX_HEIGHT // 10) if MATRIX_HEIGHT == 32 else (MATRIX_HEIGHT // 12)
    y_spacing = 10 if MATRIX_HEIGHT == 32 else 12

    # Show flights
    y_pos = 4
    for i, flight in enumerate(flights[:max_flights]):
        tail = flight.get('registration') or flight.get('callsign', '???')
        max_length = 8 if MATRIX_WIDTH <= 64 else 16
        matrixportal.add_text(
        ...
        matrixportal.set_text(tail[:max_length], i)
        y_pos += y_spacing
...
print("=" * 60)
print("Flight Portal v4.1 - Watchlist Support (Multi-Size)")
...
try:
    ...
    if free_mem < 50000:  # Less than 50KB free
        print("⚠️  WARNING: Low memory! Consider disabling HTTPS or watchlist.")
except:
    pass
